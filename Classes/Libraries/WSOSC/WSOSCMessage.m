...
+ (id)messageParsedFrom:(NSString *)data {
    return [[self alloc] initWithDataFrom:data];
}
...
//	NSLog(@"%@", tagsAndArguments);
    ...
    for (index = 0; index < [_typeTagString length]; index++) {
        switch ([_typeTagString characterAtIndex:index]) {
			case 'f':
                argumentChar = 
                [[tagsAndArguments substringWithRange:NSMakeRange(pointer,4)] UTF8String];
...
//				NSLog(@"integer %c %d", argumentChar, &argumentInt32);
...
- (NSString *)text {
	NSString *text = @"";
	if (_arguments.count > 2 && [[self addressString] isEqualToString:@"/text"]) {
		// find the char index of text arg
		// find length of text arg
		// collect together the entire text arg
	}
	return text;
}
